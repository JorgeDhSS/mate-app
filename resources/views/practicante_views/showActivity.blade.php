...
@section('body')
    <div class="border-2 gap-4 px-8 py-2 md:px-20 md:py-10 mt-10 mx-12">
        ...
        {{json_encode($actividad)}}
        <form action="{{route('practicante.sendAnswers')}}" method="post">
            @csrf
            ...
            @if($preguntaActiv->count() > 0)
                <div class="grid grid-cols-2 w-full gap-4 px-2 py-2 border-2 mt-4">
                    ...
                    @foreach($preguntaActiv as $preguntaAc)
                        <div class="w-full my-4 ml-4 col-span-2 md:col-span-2 flex flex-wrap">
                            ...
                            <div class="w-1/2">
                                ...
                                <select class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200 w-full" name="respuesta[{{$preguntaAc->id}}]">
                    ...
                    <div class="w-full my-4 ml-4 col-span-2 md:col-span-2 flex flex-wrap justify-center">
                        <button type="submit" class="bg-blue-500 rounded-lg font-bold text-white text-center px-4 py-3 transition duration-300 ease-in-out hover:bg-green-700 mr-6">Responder actividad</button>
                    </div>
                </div>
            @else
                No hay actividades
                <div class="w-full my-4 ml-4 col-span-2 md:col-span-2 flex flex-wrap justify-center">
                    <a href="" class="bg-blue-500 rounded-lg font-bold text-white text-center px-4 py-3 transition duration-300 ease-in-out hover:bg-green-700 mr-6">Otra actividad</a>
                </div>
            @endif
        </form>
    </div>
